...
<style>
    ...
    .product-card {
        height: 100%;
        min-height: 380px; /* hoặc chiều cao bạn muốn */
    }

    .product-title {
        min-height: 3.5rem; /* giữ cho tên sản phẩm luôn cao, kể cả ngắn */
    }
...
@section('content')
    <div class="container mx-auto px-4 mt-4">
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4 p-4 bg-white rounded-lg ">
            @foreach ($categories as $category)
                <a href="{{ route('exchange.categoryDetail', $category->slug) }}"
                    ...
                    <div class="w-24 h-24 shadow-md  flex items-center justify-center overflow-hidden transition duration-300">
        ...
        @if($recommended->isNotEmpty())
            <div class="relative mt-4 shadow-2xl p-5 border rounded-lg">
                ...
                <!-- Nút Prev -->
                <button id="prevBtn" class="absolute left-0 top-1/2 -translate-y-1/2 z-10 bg-white text-xl px-2 py-1 border rounded shadow">&#10094;</button>
                ...
                <div id="productContainer" class="flex overflow-hidden gap-3 scroll-smooth">
                    @foreach($recommended as $index => $product)
                        <div class="product-item w-full md:w-1/3 flex-shrink-0" data-page="{{ floor($index / 3) }}">
                            <div class="product-card border rounded-lg overflow-hidden shadow-sm hover:shadow-md hover:scale-105 transition flex flex-col">
                                <div class="relative">
                                ...
                                <div class="p-3 space-y-1 flex-1 flex flex-col justify-between">
                                    <div>
                                        <a href="{{ route('exchange.productDetail', $product['slug']) }}">
                                            <h3 class="product-title text-xl font-semibold line-clamp-2">{{ $product->name }}</h3>
                                        </a>
                ...
                <!-- Nút Next -->
                <button id="nextBtn" class="absolute right-0 top-1/2 -translate-y-1/2 z-10 bg-white px-2 py-1 text-xl border rounded shadow">&#10095;</button>
            </div>
    @endif
    ...
    <!-- Sản phẩm nổi bật -->
        ...
        <div class="bg-white p-6 rounded-xl shadow-md mt-4 mx-auto ">
            <h2 class="text-2xl font-bold text-gray-800 mb-4">Badminton Exchange - {{__("Sàn Giao Dịch Cầu Lông Secondhand Uy Tín")}}</h2>

            <div id="short-text" class="text-gray-700 space-y-3 line-clamp-7">
                <p>
                    <span class="font-semibold">Badminton Exchange</span> {{__("là nền tảng")}} <span class="text-blue-600">{{__("mua bán, trao đổi dụng cụ cầu lông secondhand")}}
                ...
                <p>
                    ...
                    {{__("khi mua các sản phẩm chất lượng từ các thương hiệu")}} <span class="text-yellow-600">Yonex, Victor, Li-Ning</span>
                    {{__("với đầy đủ phụ kiện từ vợt, giày đến túi đựng.")}}
                </p>
                <p>
                    <span class="font-medium">{{__("Đa dạng sản phẩm")}}:</span>{{__("Vợt (Astrox, Nanoflare), giày (Power Cushion), túi đựng và phụ kiện chính hãng.")}}
                </p>
                <p>
                    <span class="font-medium">{{__("Giao dịch an toàn")}}:</span> {{__("Xác minh người bán, hệ thống đánh giá uy tín, hỗ trợ gặp mặt tại sân cầu.")}}
                </p>
                <p>
                    <span class="font-medium">{{__("Dễ dàng tìm kiếm")}}:</span> {{__("Bộ lọc thông minh theo hãng, giá, tình trạng sản phẩm.")}}
                </p>
            </div>

            <div class="mt-4 flex flex-col sm:flex-row sm:items-center gap-4">
                <a href="/about-us" class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors text-center">
                    {{__("See more")}}
                </a>
                <div class="flex items-center">
                    ...
                    <span class="ml-3 text-sm text-gray-500">Đã có <span class="font-bold">1000+</span> {{__('members')}}</span>
                </div>
            </div>
        </div>
        <div class="bg-white p-6 rounded-xl shadow-md mt-4 mx-auto ">
            <div class="container my-5 ">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mt-6">
                    ...
                    <div class="bg-white p-6 rounded-xl ">
                        <div class="flex items-center">
                            <img src="{{ asset('images/upload/league/tien.jpg') }}" alt="Avatar" class="w-14 h-14 rounded-full object-cover">
                            <div class="ml-4">
                                <h5 class="text-lg font-semibold mb-0">Minh Tiến</h5>
                            </div>
                        </div>
                        <p class="mt-4 text-gray-700">
                            "An toàn, đáng tin cậy và dễ sử dụng giao diện người dùng. Nhìn chung, một cộng đồng tuyệt vời sẽ ở!"
                        </p>
                    ...
                    <div class="bg-white p-6 rounded-xl ">
                        <div class="flex items-center">
                            <img src="{{ asset('images/upload/league/vy.jpg') }}" alt="Avatar" class="w-14 h-14 rounded-full object-cover">
                            <div class="ml-4">
                                <h5 class="text-lg font-semibold mb-0">Thảo Vy</h5>
                            </div>
                        </div>
                        <p class="mt-4 text-gray-700">
                            "Nền tảng dễ dàng và thuận tiện. Nó thuận tiện để mua và bán đồ. Hãy trò chuyện trực tiếp với các băng chuyền khác."
                        </p>
                        <span class="text-yellow-400 text-xl mt-2 block">★★★★★</span>
                    </div>
                    ...
                    <div class="bg-white p-6 rounded-xl ">
                        <div class="flex items-center">
                            <img src="{{ asset('images/upload/league/tung.jpg') }}" alt="Avatar" class="w-14 h-14 rounded-full object-cover">
                            <div class="ml-4">
